$(function () {
    ...
    $(window).bind('scroll', function () {
        setTimeout(function () {
            $('#share').animate({
                attr: 'y',
                target: parseInt(getScroll().top + (getInner().height - parseInt(getStyle($('#share').first(), 'height'))) / 2),
                step: 50,
            })
        }, 100)
    })
    ...
    function _wait_load() {
        setTimeout(function () {
            for (var i = 0; i < lazyload.length(); i++) {
                ...
                var _this = lazyload.ge(i);
                if (_this.imgLoad == undefined) {
                    if (clientHeight + scrollTop >= getOffsetTop($(_this).first())) {
    ...
    function containNextImgSrc(_this) {
        ...
        prev_img.src = $('.lazyload').eq(prev).attr('xsrc');
        prev_img.index = prev;
    ...
    }).drag($('#photo_big h2').getElement(0));
