    ...
    for(var i = 0;i<this.elements.length;i++) {
        ...
        timer = setInterval(function() {
            if(type == 'buffer') {
                console.log()
                if(attr == 'opacity') {
                    step = step > 0 ? Math.ceil((target - parseFloat(getStyle(element, attr)) * 100 ) / speed) : Math.floor((target - parseFloat(getStyle(element, attr)) * 100 ) / speed)
                } else {
                    step = step > 0 ? Math.ceil((target - parseInt(getStyle(element, attr))) / speed) : Math.floor((target - parseInt(getStyle(element, attr))) / speed)
                }
            }
            ...
            if(attr == 'opacity') {
                ...
                // var temp;
                // if(target < getStyle(element, attr) * 100) {
                //     temp = getStyle(element, attr) * 100 +step;
                // } else {
                //     temp = getStyle(element, attr) * 100 + step;
                // }
                // element.style.opacity = parseFloat(temp) / 100;
                // if(element.style.opacity == (target / 100)) {
                //     clearInterval(timer)
                // }
                // console.log(target)
                // console.log(getStyle(element, attr))
            } else {
                if(step < 0 && parseInt(getStyle(element, attr)) + step < target ){
                    setTarget()
