...
Base.prototype.center = function (width, height) {
    var top = (getInner().height - height) / 2 + getScroll().top + 'px';
    var left = (getInner().width - width) / 2 + getScroll().left + 'px';
    for (var i = 0; i < this.elements.length; i++) {
...
Base.prototype.resize = function (fn) {
    for (var i = 0; i < this.elements.length; i++) {
        ...
        addEvent(window, 'resize', function () {
            ...
            } else if  (element.offsetLeft + element.clientWidth > getInner().width - getScroll().left) {
                element.style.left = getInner().width - element.clientWidth + getScroll().left - 50 + 'px';
            }
            if(element.offsetTop <= 0 + getScroll().top) {
                element.style.top = 0 + getScroll().top + 'px'
            } else if(element.offsetTop + element.clientHeight > getInner().height - getScroll().top) {
                element.style.top = getInner().height - element.clientHeight + getScroll().top - 50 + 'px';
            }
        })
    }
    return this
